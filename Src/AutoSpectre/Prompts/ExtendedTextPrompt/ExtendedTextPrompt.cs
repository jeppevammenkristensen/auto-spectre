...
public sealed class ExtendedTextPrompt<T> : IPrompt<T>, IHasCulture
{
    ...
    /// <summary>
    /// Gets or sets a value indicating if the help text
    /// should be displayed (if it is not null)
    /// </summary>
    ...
    public ExtendedTextPrompt(string prompt, StringComparer? comparer = null)
    {
    ...
    private void WritePrompt(IAnsiConsole console)
    {
        ...
        if (ShowHelp && HelpText != null)
        {
            ...
            builder.AppendFormat(CultureInfo.InvariantCulture, " [{0}]({1})[/]", helpStyle, HelpText);
        }
