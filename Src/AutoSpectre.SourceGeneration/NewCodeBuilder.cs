...
/// <summary>
/// Handles building the code for with the analyzed data. Generates <br/> 
/// The Concrete Factories <br/>
/// The SpectreFactory <br/>
/// The summaries override if necessary
/// </summary>
internal class NewCodeBuilder
{
    ...
    public bool BuildPartialCode(out string partialCode)
    {
        partialCode = string.Empty;

        if (StepContexts.All(x => !x.GetConditionContext().IsPartial))
        {
            return false;
        }
        ...
        using (NamespaceScope.Begin(builder, Type.ContainingNamespace))
        {
            builder.AppendLine($"\tpublic partial class {Type.Name}")
                .AppendLine($"\t{{");

            foreach (var stepContext in StepContexts.Where(x => x.GetConditionContext().IsPartial))
            {
                var summaryBuilder = new SummaryLineWriter(builder);

                summaryBuilder.AppendLine("/// <summary>", false);
                stepContext.BuildContext.WriteToSummary(summaryBuilder);
                summaryBuilder.AppendLine("/// </summary>", false);

                MemberDeclarationSyntax syntax = stepContext switch
                {
                    MethodContext methodContext => methodContext.EvaluationContext.MethodSyntax,
                    PropertyContext propertyContext => (MemberDeclarationSyntax) propertyContext.PropertySymbol
                        .DeclaringSyntaxReferences[0].GetSyntax(),
                    _ => throw new ArgumentOutOfRangeException(nameof(stepContext))
                };
                ...
                else if (syntax is MethodDeclarationSyntax method)
                {
                    ...
                    builder.AppendLine(method.NormalizeWhitespace().ToFullString());
                }
            }
        ...
        partialCode = ParseCompilationUnit(builder.ToString()).NormalizeWhitespace().ToFullString();
        return true;
    }
    ...
    public string BuildCode()
    {
        ...
        var implementationInheritance = isAsync
            ? $"AsyncSpectreFactoryBase<{typeName}>, {SpectreFactoryInterfaceName}"
            : $"{SpectreFactoryInterfaceName}";
        ...
        return SyntaxFactory.ParseCompilationUnit(builder.ToString()).NormalizeWhitespace().ToFullString();
    }
...
//     }
    ...
    private bool EvaluateIfCanBeInitalizedWithoutParameters()
    {
        ...
        return SingleFormEvaluationContext.UsedConstructor!.Parameters.Length == 0 &&
               !Type.GetAllProperties().Any(x => x.IsRequired);
    }
    ...
    {
        var builder = new StringBuilder();
        foreach (var nmSpace in InitialNamespaces.Concat(StepContexts.SelectMany(x => x.BuildContext.Namespaces()))
                     .Distinct().Where(x => x != Type.ContainingNamespace.ToString()))
        {
            ...
            void AddLine()
            {
                switch (stepContext)
                {
                    ...
                    case PropertyContext propertyContext:
                        builder.AppendLine(
                            $"{propertyContext.BuildContext.GenerateOutput($"{FormName}.{propertyContext.PropertyName}")}");
                        break;
                    default:
    ...
    {
        var containingUsing = Type.ContainingNamespace.IsGlobalNamespace
            ? string.Empty
    ...
    private string GenerateSpectreFactoryMethodInvocations()
    {
        ...
        builder.AppendLine(
            $"public static {spectreInterfaceType} {getSpectreFactoryMethodName}() => new {SpectreFactoryClassName}();");
        ...
        {
            builder.AppendLine(
                $"public static {asyncString} {returnType} {methodName}_{typeNameWithUnderscore}({typeName} result)");
            builder.AppendLine("{");
            builder.AppendLine($"\tvar factory = {getSpectreFactoryMethodName}();");
            builder.AppendLine($"\treturn {awaitString} factory.{methodName}(result);");
            builder.AppendLine("}");
